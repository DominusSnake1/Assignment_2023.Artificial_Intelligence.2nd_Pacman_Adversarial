...
#     state.getNumAgents()
#     state.isWinningState()
#     state.isLosingState()
#     state.generateNextState(agentIndex, action)
#     state.getScore()
...
class MultiagentTreeState(object):
    ...
    def generateNextState(self, agentIndex, action):
        if VERBOSE:
            print("generateNextState(%s, %s, %s) -> %s" % (self.state, agentIndex,
...
def run(lay, layName, pac, ghosts, disp, nGames=1, name='games'):
    ...
    stats = {'time': time.time() - starttime, 'wins': [g.state.isWinningState() for g in games].count(True), 'games': games, 'scores': [g.state.getScore() for g in games],
...
class EvalAgentTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        stats = {'time': totalTime, 'wins': [g.state.isWinningState() for g in games].count(True),
